 ...
 * all_tasks_results_return_now() once it is done if it is done
 * with whatever would add async tasks and no async tasks were actually
 * added. That will cause the closure to return on the next idle if there
 * were no tasks to be done. Otherwise, #AllTasksResultsClosure will
 * wait indefinitely for a task to be added.
 *
